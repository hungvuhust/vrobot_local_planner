...
void RPPController::configure(
    const rclcpp::Node::WeakPtr &parent, std::string name,
  ...
  RCLCPP_INFO(node->get_logger(), "RPPController configured");
  ...
  node->declare_parameter(plugin_name_ + ".desired_linear_vel", 0.5);
  node->declare_parameter(plugin_name_ + ".lookahead_dist", 0.6);
  node->declare_parameter(plugin_name_ + ".min_lookahead_dist",
                          rclcpp::ParameterValue(0.3));
  node->declare_parameter(plugin_name_ + ".max_lookahead_dist", 0.9);
  node->declare_parameter(plugin_name_ + ".lookahead_time", 1.5);
  node->declare_parameter(plugin_name_ + ".rotate_to_heading_angular_vel", 1.8);
  node->declare_parameter(plugin_name_ + ".transform_tolerance", 0.1);
  node->declare_parameter(plugin_name_ + ".use_velocity_scaled_lookahead_dist",
                          false);
  node->declare_parameter(plugin_name_ + ".min_approach_linear_velocity", 0.05);
  node->declare_parameter(plugin_name_ + ".approach_velocity_scaling_dist",
                          0.6);
  node->declare_parameter(
      plugin_name_ + ".max_allowed_time_to_collision_up_to_carrot", 1.0);
  node->declare_parameter(plugin_name_ + ".use_collision_detection", true);
  node->declare_parameter(
      plugin_name_ + ".use_regulated_linear_velocity_scaling", true);
  node->declare_parameter(
      plugin_name_ + ".use_cost_regulated_linear_velocity_scaling", true);
  node->declare_parameter(plugin_name_ + ".cost_scaling_dist", 0.6);
  node->declare_parameter(plugin_name_ + ".cost_scaling_gain", 1.0);
  node->declare_parameter(plugin_name_ + ".inflation_cost_scaling_factor", 3.0);
  node->declare_parameter(plugin_name_ + ".regulated_linear_scaling_min_radius",
                          0.90);
  node->declare_parameter(plugin_name_ + ".regulated_linear_scaling_min_speed",
                          0.25);
  node->declare_parameter(plugin_name_ + ".use_rotate_to_heading", true);
  node->declare_parameter(plugin_name_ + ".rotate_to_heading_min_angle", 0.785);
  node->declare_parameter(plugin_name_ + ".max_angular_accel", 3.2);
  node->declare_parameter(plugin_name_ + ".allow_reversing", false);
  node->declare_parameter(plugin_name_ + ".max_robot_pose_search_dist",
                          getCostmapMaxExtent());
  node->declare_parameter(plugin_name_ + ".use_interpolation", true);
  ...
  global_path_pub_ = node->create_publisher<vrobot_local_planner::msg::Path>(
...
double
RPPController::getLookAheadDistance(const geometry_msgs::msg::Twist &speed) {
...
geometry_msgs::msg::TwistStamped RPPController::computeVelocityCommands(
    ...
    vrobot_local_planner::GoalChecker            *goal_checker) {
  ...
  if (shouldRotateToGoalHeading(carrot_pose)) {
    ...
    applyConstraints(curvature, speed,
...
bool RPPController::shouldRotateToPath(
    const vrobot_local_planner::msg::PlannerPose &carrot_pose,
...
RPPController::circleSegmentIntersection(const geometry_msgs::msg::Point &p1,
...
vrobot_local_planner::msg::PlannerPose RPPController::getLookAheadPoint(
...
bool RPPController::isCollisionImminent(
    const vrobot_local_planner::msg::PlannerPose &robot_pose,
...
bool RPPController::inCollision(const double &x, const double &y,
                                const double &theta) {
  unsigned int mx, my;

  if (!costmap_->worldToMap(x, y, mx, my)) {
    RCLCPP_WARN_THROTTLE(logger_, *(clock_), 30000,
                         "The dimensions of the costmap is too small to "
                         "successfully check for "
                         "collisions as far ahead as requested. Proceed at "
                         "your own risk, slow "
                         "the robot, or "
...
double RPPController::costAtPose(const double &x, const double &y) {
  unsigned int mx, my;
  ...
  if (!costmap_->worldToMap(x, y, mx, my)) {
    ...
    throw std::runtime_error(
        "RPPController: Dimensions of the costmap are too small to "
        "encapsulate "
        "the robot footprint at current speeds!");
  }
...
double RPPController::approachVelocityScalingFactor(
    const vrobot_local_planner::msg::Path &transformed_path) const {
  ...
  if (remaining_distance < approach_velocity_scaling_dist_) {
    ...
    double distance_to_last_pose =
        std::hypot(last.pose.position.x, last.pose.position.y);
...
void RPPController::applyApproachVelocityScaling(
    const vrobot_local_planner::msg::Path &path, double &linear_vel) const {
...
void RPPController::applyConstraints(
    ...
    const double &pose_cost, const vrobot_local_planner::msg::Path &path,
...
void RPPController::setPlan(const vrobot_local_planner::msg::Path &path) {
  global_plan_ = path;
}

void RPPController::setSpeedLimit(const double &speed_limit,
...
vrobot_local_planner::msg::Path RPPController::transformGlobalPlan(
    const vrobot_local_planner::msg::PlannerPose &pose) {
  ...
  vrobot_local_planner::msg::PlannerPose robot_pose;
  ...
  auto closest_pose_upper_bound =
      vrobot_local_planner::geometry_utils::first_after_integrated_distance(
  ...
  auto transformation_begin = vrobot_local_planner::geometry_utils::min_by(
      global_plan_.poses.begin(), closest_pose_upper_bound,
      [&robot_pose](const vrobot_local_planner::msg::PlannerPose &ps) {
        return vrobot_local_planner::geometry_utils::euclidean_distance(
            robot_pose, ps);
      });
  ...
  auto transformation_end = std::find_if(
      transformation_begin, global_plan_.poses.end(), [&](const auto &pose) {
        return vrobot_local_planner::geometry_utils::euclidean_distance(
                   pose, robot_pose) > max_costmap_extent;
      });

  // Lambda to transform a PoseStamped from global frame to local
  auto transformGlobalPoseToLocal = [&](const auto &global_plan_pose) {
    vrobot_local_planner::msg::PlannerPose stamped_pose, transformed_pose;
    stamped_pose                 = global_plan_pose;
  ...
  // reference.
  vrobot_local_planner::msg::Path transformed_plan;
...
double RPPController::findVelocitySignChange(
    const vrobot_local_planner::msg::Path &transformed_plan) {
  ...
  for (unsigned int pose_id = 1; pose_id < transformed_plan.poses.size() - 1;
    ...
    and determine it's distance from the robot. If there is no cusp in the
    path, then just determine the distance to the goal location. */
...
bool RPPController::transformPose(
  ...
  geometry_msgs::msg::PoseStamped in_pose_stamped, out_pose_stamped;

  in_pose_stamped.pose   = in_pose.pose;
  in_pose_stamped.header = in_pose.header;

  try {
    out_pose = in_pose;
    tf_->transform(in_pose_stamped, out_pose_stamped, frame,
                   transform_tolerance_);
    out_pose.header.frame_id = frame;
    out_pose.pose            = out_pose_stamped.pose;
    return true;
...
double RPPController::getCostmapMaxExtent() const {
...
PLUGINLIB_EXPORT_CLASS(vrobot_local_planner::RPPController,
                       vrobot_local_planner::Controller)
