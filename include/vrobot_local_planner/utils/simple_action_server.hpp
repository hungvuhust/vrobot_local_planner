...
#include "vrobot_local_planner/utils/node_thread.hpp"
...
public:
  ...
  explicit SimpleActionServer(
    ...
    if (spin_thread_) {
      ...
      executor_thread_ =
          std::make_unique<vrobot_local_planner::NodeThread>(executor_);
    }
  }
...
protected:
  ...
  std::unique_ptr<vrobot_local_planner::NodeThread>    executor_thread_;
