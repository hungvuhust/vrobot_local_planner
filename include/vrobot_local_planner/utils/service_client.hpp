...
/**
 * @brief A simple service client that can be used to call services
 * @tparam ServiceT The type of the service to call
 * @tparam NodeT The type of the node to create the service client off of
 */
    ...
    for (uint8_t i = 0; i < max_retries; ++i) {
      try {
        ...
        if (callback_group_executor_.spin_until_future_complete(
          ...
          throw std::runtime_error(
              service_name_ + " service client: async_send_request failed");
        }

        return future_result.get();
      } catch (const std::exception &e) {
        RCLCPP_ERROR(node_->get_logger(),
                     "%s service client: %s, retrying (%d/%d)",
                     service_name_.c_str(), e.what(), i + 1, max_retries);
        if (i == max_retries - 1) {
          return nullptr; // Return nullptr if all retries failed
        }
